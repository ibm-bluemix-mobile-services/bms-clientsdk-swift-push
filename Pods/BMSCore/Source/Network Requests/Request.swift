...
*/
open class Request: NSObject, URLSessionTaskDelegate {
    ...
    */
    public func send(requestBody: Data? = nil, completionHandler: BMSCompletionHandler?) {
        ...
        if let url = URL(string: self.resourceUrl) {
            ...
            Request.logger.error(message: urlErrorMessage)
    ...
    private func buildAndSendRequest(url: URL, callback: BMSCompletionHandler?) {
        ...
        if queryParameters != nil {
            guard let urlWithQueryParameters = Request.append(queryParameters: queryParameters!, toURL: requestUrl) else {
        ...
        Request.logger.debug(message: "Sending Request to " + resourceUrl)
    ...
    */
    ...
    public func urlSession(_ session: URLSession,
        ...
        if allowRedirects {
            Request.logger.debug(message: "Redirecting: " + String(describing: session))
...
/**
    ...
    `Request` is a simpler alternative to `BMSURLSession` that requires no familiarity with Swift's [NSURLSession](https://developer.apple.com/reference/foundation/urlsession) API.

    When building a Request object, all components of the HTTP request must be provided in the initializer, except for the `requestBody`, which can be supplied as NSData when sending the request via `send(requestBody:completionHandler:)`.
*/
...
public class Request: NSObject, NSURLSessionTaskDelegate {
    ...
    */
    ...
    public func URLSession(session: NSURLSession,
        ...
        if allowRedirects {
            Request.logger.debug(message: "Redirecting: " + String(session))
            redirectRequest = request
        }
